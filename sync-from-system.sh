...
sync_backgrounds() {
    local repo_backgrounds="$SCRIPT_DIR/backgrounds"
    local system_backgrounds="/usr/share/backgrounds"

    if [[ ! -d "$repo_backgrounds" ]]; then
        log_warning "Backgrounds directory not in repo (skipping)"
        return
    fi

    if [[ "$DRY_RUN" == "true" ]]; then
        echo -e "${YELLOW}[DRY-RUN]${NC} Would sync backgrounds from $system_backgrounds"
        return
    fi

    log_info "Syncing backgrounds (requires sudo)..."

    if ! sudo -v; then
        log_error "sudo access required for backgrounds"
        return 1
    fi

    # Sync only files that exist in the repo (don't pull all system backgrounds)
    for file in "$repo_backgrounds"/*; do
        local filename
        filename=$(basename "$file")
        if [[ -f "$system_backgrounds/$filename" ]]; then
            sudo cp "$system_backgrounds/$filename" "$repo_backgrounds/"
            sudo chown "$USER:$USER" "$repo_backgrounds/$filename"
        fi
    done

    log_success "Backgrounds synced"
}
...
main() {
    ...
    if [[ -d "$SCRIPT_DIR/backgrounds" ]]; then
        if [[ "$DRY_RUN" == "true" ]] || prompt_yes_no "Sync backgrounds? (requires sudo)" "y"; then
            sync_backgrounds
        fi
    fi
